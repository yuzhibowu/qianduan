...
const colorTargetLabel = (target: ColorTarget) => target === "keynote" ? "Keynote" : "无边记"
...
export default function App() {
  ...
  const usdzPayload = useMemo(() => ({ ...exportPayload, colorComp: colorCorrection ? activeColorProfile : undefined, emissiveLift, unlit }), [exportPayload, colorCorrection, activeColorProfile, emissiveLift, unlit])
  const colorMismatch = (activeColorProfile.calibratedUnlit !== null && activeColorProfile.calibratedUnlit !== unlit) || (activeColorProfile.calibratedLift !== null && Math.abs(activeColorProfile.calibratedLift - emissiveLift) > 0.02)
    ...
    <aside className="side">
      ...
      <div className="side-body">
        ...
        <section className="export-block">
          <button className="btn-primary mov" disabled={job.running} onClick={() => startExport("mov")}>{job.running ? "正在导出…" : "导出透明 MOV"}</button>
          <button className="btn-primary apng" disabled={job.running} onClick={() => startExport("apng")}>{job.running ? "正在导出…" : "导出 PNG 动图"}</button>
          <label className="check-row"><input type="checkbox" checked={pngCompression} onChange={(event) => setPngCompression(event.target.checked)} />无损压缩 PNG 动图</label>
          {job.running && <button className="btn cancel" onClick={cancelExport}>取消导出</button>}
          <div className="progress"><span style={{ width: `${job.progress}%` }} /></div>
          <p className="status">{job.stage}{job.totalFrames > 0 ? ` · ${job.frame} / ${job.totalFrames} · ${Math.round(job.progress)}%` : ""}</p>
          {job.error && <p className="error">{job.error}</p>}
          {job.outputPath && <><p className="path">{job.outputPath}</p><button className="btn" onClick={() => revealOutput()}>在 Finder 中显示</button></>}
          <div className="format-divider"><span>Keynote 原生 3D</span></div>
          ...
          <div className="color-correction">
            ...
            <details className="color-tweaks">
              ...
              <Slider label="补光（换取更亮的颜色上限）" value={Math.round(emissiveLift * 100)} min={0} max={100} step={5} display={`${Math.round(emissiveLift * 100)}%`} onChange={(value) => setEmissiveLift(value / 100)} />
              <p className="color-hint">补光会用同一份颜色同时写入自发光，提高亮色上限；加得越多，立体明暗会越平。改动后需要重新校准。</p>
              <label className="check-row"><input type="checkbox" checked={unlit} onChange={(event) => setUnlit(event.target.checked)} /><span>完全关掉灯光（补光拉满）</span></label>
              {colorMismatch && <p className="color-warning">当前内置校准是在补光 {Math.round((activeColorProfile.calibratedLift ?? 0) * 100)}%、灯光{activeColorProfile.calibratedUnlit ? "关着" : "开着"}时测得；当前设置不同，颜色会偏。</p>}
              <p className="color-hint">当前：{activeColorProfile.calibratedAt ?? "无校准"}。这是与三棱柱项目同步的 117 格实测校准档。</p>
              <button className="btn reset-profile" onClick={() => { setEmissiveLift(activeColorProfile.calibratedLift ?? 0.5); setUnlit(activeColorProfile.calibratedUnlit ?? false) }}>恢复内置校准</button>
            </details>
          </div>
          {usdzJob.summary && <p className="status">{usdzJob.summary}</p>}
          {usdzJob.error && <p className="error">{usdzJob.error}</p>}
          {usdzJob.outputPath && <><p className="path">{usdzJob.outputPath}</p><button className="btn" onClick={() => revealOutput(usdzJob.outputPath)}>在 Finder 中显示 USDZ</button></>}
        </section>
      </div>
    </aside>
  </main>
