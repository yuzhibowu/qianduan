...
这是“前端→Keynote”网页的本机导出桥梁。助手只监听 `127.0.0.1:43987`，默认只接受 `https://qianduan.vercel.app`、`http://127.0.0.1:*` 和 `http://localhost:*`。它不接受任意终端命令，只提供能力检测、开始导出、追加 PNG 帧、完成、下载和取消六类固定操作。
